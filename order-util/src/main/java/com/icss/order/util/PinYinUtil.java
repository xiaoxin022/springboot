...
/**
 * Created by Linxingyu on 2018/8/7.
 */
public class PinYinUtil {
     ...
     * 转换全部大写
     *
     * @param link 不同字符串转换后,链接的字符串
     * @param strs 字符串
     * @return strs为 颐和园  一号,link为_ return获取到的是YHY_YH
     */
    public static String toPinYinUppercaseMulti(String link, String... strs)
            throws BadHanyuPinyinOutputFormatCombination {
        StringBuilder builder = new StringBuilder();
        int length = strs.length;
        for (int i = 0; i < length; i++) {
            builder.append(toPinYin(strs[i], "", Type.UPPERCASE));
            if (i < length - 1) {
                builder.append(link);
            }
        }
