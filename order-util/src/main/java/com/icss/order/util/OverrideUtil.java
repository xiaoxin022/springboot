 ...
 * @description: 掩码工具类
 * @author: Linxingyu
 * @create: 2018/09/10 11:16
 */
public class OverrideUtil {

    /**
     * 掩码缓存，当所需掩码数量在0-16范围之内的时候 直接使用缓存,不需要创建内存对象，效率最高
     * <p>
     ...
     */
    public OverrideUtil(String initialCode, int beginIndex, int endIndex) {
     ...
     * @param endIndex 结束掩码位置
     */
    public OverrideUtil(String initialCode, int beginIndex, int endIndex, char cover) {
        this.initialCode = initialCode;
        this.beginIndex = beginIndex;
        this.endIndex = endIndex;
        this.cover = cover;
    ...
    /**
     * 提供四种静态方法（静态方法调用实例方法） 两种实例方法
     */
    public static String getMaskSubWay(String initialCode, int beginIndex, int endIndex) {
        OverrideUtil override = new OverrideUtil(initialCode, beginIndex, endIndex);
        return override.getMaskSubWay();
    }

    public static String getMaskSubWay(String initialCode, int beginIndex) {
        if (initialCode != null && initialCode.trim().length() > 0) {

            String substring = initialCode.substring(beginIndex);
            String result = initialCode.replace(substring, cache[1]);
            return result;
        }
        return null;
    }
    ...
    public static String getMaskSubWay(String initialCode, int beginIndex, int endIndex,
            char cover) {
        OverrideUtil override = new OverrideUtil(initialCode, beginIndex, endIndex, cover);
        return override.getMaskSubWay();
    }
